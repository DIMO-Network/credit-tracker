...
func teardownIfLastTest(t *testing.T) {
	globalTestContainer.refs.Add(1)
	t.Cleanup(func() {
		refs := globalTestContainer.refs.Add(-1)
		if refs != 0 {
			return
		}
...
func setupTestContainer(t *testing.T) *sql.DB {
	globalTestContainer.onceSetup.Do(func() {
		...
		globalTestContainer.db, err = sql.Open("postgres", dbSettings.BuildConnectionString(true))
		require.NoError(t, err)

		err = migrations.RunGoose(ctx, []string{"up"}, dbSettings)
		require.NoError(t, err)
	})

	return globalTestContainer.db
}
...
func TestDeductCredits(t *testing.T) {
	...
	t.Run("deduction with zero amount", func(t *testing.T) {
		...
		err = repo.DeductCredits(ctx, licenseID, testAssetID, 0, testAPIEndpoint, uuid.NewString())
		require.NoError(t, err)
	...
	t.Run("deduction with negative amount", func(t *testing.T) {
		...
		// Test: Try to deduct negative amount
		err = repo.DeductCredits(ctx, licenseID, testAssetID, -10, testAPIEndpoint, uuid.NewString())
		require.Error(t, err)
...
func TestRefundCredits(t *testing.T) {
	...
	t.Run("successful refund", func(t *testing.T) {
		...
		err = repo.DeductCredits(ctx, licenseID, testAssetID, 100, testAPIEndpoint, referenceID)
		require.NoError(t, err)
	...
	t.Run("refund with multiple grants", func(t *testing.T) {
		...
		referenceID := uuid.NewString()
		err = repo.DeductCredits(ctx, licenseID, testAssetID, 10, testAPIEndpoint, referenceID)
		require.NoError(t, err)
	...
	t.Run("refund with zero amount", func(t *testing.T) {
		...
		referenceID := uuid.NewString()
		err = repo.DeductCredits(ctx, licenseID, testAssetID, 0, testAPIEndpoint, referenceID)
		require.NoError(t, err)
...
func TestCreateGrant(t *testing.T) {
	...
	t.Run("successful grant creation", func(t *testing.T) {
		...
		err := repo.CreateGrant(ctx, licenseID, testAssetID, defaultGrantAmount, testTXHash, time.Now())
		require.NoError(t, err)
	...
	t.Run("create grant with existing debt", func(t *testing.T) {
		...
		// Test: Create a new grant
		err = repo.CreateGrant(ctx, licenseID, testAssetID, defaultGrantAmount, localTextTXHash.Hex(), time.Now())
		require.NoError(t, err)
	...
	t.Run("create grant with zero amount", func(t *testing.T) {
		...
		err := repo.CreateGrant(ctx, licenseID, testAssetID, 0, testTXHash, time.Now())
		require.Error(t, err)
	...
	t.Run("create grant with negative amount", func(t *testing.T) {
		...
		// Test: Try to create a grant with negative amount
		err := repo.CreateGrant(ctx, licenseID, testAssetID, -100, testTXHash, time.Now())
		require.Error(t, err)
...
func TestConfirmGrant(t *testing.T) {
	...
	t.Run("successful grant confirmation", func(t *testing.T) {
		...
		err = repo.ConfirmGrant(ctx, licenseID, testAssetID, localTextTXHash.Hex(), 100, defaultGrantAmount, time.Now())
		require.NoError(t, err)
...
func TestFIFOOrdering(t *testing.T) {
	...
	t.Run("deduction follows FIFO order", func(t *testing.T) {
		...
		err := repo.ConfirmGrant(ctx, licenseID, testAssetID, localTextTXHash.Hex(), 1, defaultGrantAmount, time.Now().Add(-time.Hour))
		require.NoError(t, err)
		err = repo.ConfirmGrant(ctx, licenseID, testAssetID, localTextTXHash2.Hex(), 2, defaultGrantAmount, time.Now())
		require.NoError(t, err)
	...
	t.Run("FIFO with multiple grants", func(t *testing.T) {
		...
		err := repo.ConfirmGrant(ctx, licenseID, testAssetID, localTextTXHash.Hex(), 1, defaultGrantAmount, time.Now().Add(-time.Hour*2))
		require.NoError(t, err)
		err = repo.ConfirmGrant(ctx, licenseID, testAssetID, localTextTXHash2.Hex(), 2, defaultGrantAmount, time.Now().Add(-time.Hour))
		require.NoError(t, err)
		err = repo.ConfirmGrant(ctx, licenseID, testAssetID, localTextTXHash3.Hex(), 3, defaultGrantAmount, time.Now())
		require.NoError(t, err)
...
func TestConcurrentOperations(t *testing.T) {
	...
	t.Run("concurrent deductions", func(t *testing.T) {
		...
		err := repo.ConfirmGrant(ctx, licenseID, testAssetID, localTextTXHash.Hex(), 1, defaultGrantAmount, time.Now())
		require.NoError(t, err)

		// Test: Perform concurrent deductions
		done := make(chan error, 2)
		go func() {
			done <- repo.DeductCredits(ctx, licenseID, testAssetID, 300, testAPIEndpoint, uuid.NewString())
		}()
		go func() {
			done <- repo.DeductCredits(ctx, licenseID, testAssetID, 400, testAPIEndpoint, uuid.NewString())
		}()
	...
	t.Run("concurrent refunds", func(t *testing.T) {
		...
		err = repo.DeductCredits(ctx, licenseID, testAssetID, 1000, testAPIEndpoint, referenceID1)
		require.NoError(t, err)

		referenceID2 := uuid.NewString()
		err = repo.DeductCredits(ctx, licenseID, testAssetID, 200, testAPIEndpoint, referenceID2)
		require.NoError(t, err)

		// Test: Perform concurrent refunds
		done := make(chan error, 2)
		go func() {
			done <- repo.RefundCredits(ctx, testAPIEndpoint, referenceID1)
		}()
		go func() {
			done <- repo.RefundCredits(ctx, testAPIEndpoint, referenceID2)
		}()
	...
	t.Run("concurrent grant confirmations", func(t *testing.T) {
		...
		go func() {
			done <- repo.ConfirmGrant(ctx, licenseID, testAssetID, localTextTXHash.Hex(), 1, defaultGrantAmount, time.Now())
		}()
		go func() {
			done <- repo.ConfirmGrant(ctx, licenseID, testAssetID, localTextTXHash.Hex(), 2, defaultGrantAmount, time.Now())
		}()

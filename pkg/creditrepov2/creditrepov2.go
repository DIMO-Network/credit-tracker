...
func (r *Repository) DeductCredits(ctx context.Context, licenseID, assetDID string, amount int64, appName, referenceID string) error {
	if amount < 0 {
		return fmt.Errorf("invalid amount: %d. Amount must be positive", amount)
	}

	// First check for outstanding debt from failed grants
	debt, err := r.getOutstandingDebt(ctx, licenseID, assetDID)
	if err != nil {
		return fmt.Errorf("failed to check outstanding debt: %w", err)
	}
	...
	if currentBalance < amount {
		return fmt.Errorf("insufficient credits. Current: %d, Required: %d", currentBalance, amount)
	}
	...
	for _, grant := range grants {
		...
		if err := opGrant.Insert(ctx, tx, boil.Infer()); err != nil {
			return fmt.Errorf("failed to record operation grant: %w", err)
		}
...
func (r *Repository) RefundCredits(ctx context.Context, appName, referenceID string) error {
	...
	for _, opGrant := range grants {
		...
		if grant == nil {
			return fmt.Errorf("grant not found for operation grant %s", opGrant.ID)
		}
	...
	if err != nil {
		return fmt.Errorf("failed to settle debt: %w", err)
	}
...
func (r *Repository) CreateGrant(ctx context.Context, licenseID, assetDID string, amount int64, txHash string, mintTime time.Time) error {
	if amount <= 0 {
		return fmt.Errorf("invalid amount: %d. Amount must be positive", amount)
	}
	...
	if err := grant.Insert(ctx, tx, boil.Infer()); err != nil {
		return fmt.Errorf("failed to create grant record: %w", err)
	}
...
// 2. Create a new operation record
// 3. Settle any debt if any
func (r *Repository) ConfirmGrant(ctx context.Context, licenseID, assetDID string, txHash string, logIndex int, creditAmount int64, mintTime time.Time) error {
	tx, err := r.db.BeginTx(ctx, &sql.TxOptions{
		Isolation: sql.LevelReadCommitted,
	})
	if err != nil {
	...
	if err := opGrant.Insert(ctx, tx, boil.Infer()); err != nil {
		return fmt.Errorf("failed to record grant operation: %w", err)
	}
